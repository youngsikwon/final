  ...
  recognition.onresult = function(event) {
    ...
    for (var i = event.resultIndex; i < event.results.length; ++i) {
      if (event.results[i].isFinal) {
        ...
        if(answer.replace(/\s/gi, "") == "3조자기소개"){
         	answer = "";
        	speak("안녕하세요 저는 지니에요");
        }
        else if(answer.replace(/\s/gi, "") == "3조사이트에대해서소개해줘"){
        	answer = "";
        	speak("커프링크는 파트너스가 프로젝트를 등록하고 개발자가 신청하여");
        }
        else if(answer.replace(/\s/gi, "") == "3조사이트에대해소개해줘"){
        	answer = "";
        	speak("커프링크는 파트너스가 프로젝트를 등록하고 개발자가 신청하여");
        }
        else if(answer.replace(/\s/gi, "") == "3조오늘날씨"){
        	answer = "";
        	speak("오늘 날씨는 맑음");
        }
        else if(answer.replace(/\s/gi, "") == "3조네이버들어가"){
        	answer = "";
